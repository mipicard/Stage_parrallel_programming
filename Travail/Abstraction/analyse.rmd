...
stat<-ddply(data,c("Algo", "TailleMat", "RunId", "Temps"),summarise,N=length(Temps),mean=mean(Temps),sd=sd(Temps),se=sd/sqrt(N))
# Here we have two parameters monitor that appears on x
# and Log that is used for color and groupping
p <- ggplot(stat,aes(x=TailleMat, y=Temps, colour=Algo, shape=Algo))
